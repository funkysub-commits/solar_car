...
export CLOCK_TICK="$(bashio::config 'clock_tick')"
...
export ENT_ODOMETER="$(bashio::config 'ent_odometer')"
export AUX_ENABLED="$(bashio::config 'aux_enabled')"
export ENT_AUX_SOC="$(bashio::config 'ent_aux_soc')"
export ENT_AUX_STATUS="$(bashio::config 'ent_aux_status')"
# aux low-battery alarm: SoC levels to sound at, the sound file, and the player
export AUX_LOW_LEVELS="$(bashio::config 'aux_low_levels' '')"
export AUX_LOW_SOUND="$(bashio::config 'aux_low_sound' '')"
export AUX_ALARM_PLAYER="$(bashio::config 'aux_alarm_player' '')"
