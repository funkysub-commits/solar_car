...
the battery (or the SH-C31G adapter) isn't connected. No device is
opened and no log file is written in that mode.
...
Talks to gs_usb directly (same reason as ezkontrol_decode.py: python-can
4.6.1 can't compute valid timing for this adapter's 170 MHz CAN clock).
For 500 kbps we set the bit-timing registers explicitly:

    sync=1, prop=1, phase1=13, phase2=2, sjw=2, brp=20
    => tq=17, sample point = (1+1+13)/17 = 88.2%
    => bitrate = 170e6 / (20 * 17) = 500000 bps exactly
...
# State + change tracking (same pattern as ezkontrol_decode.py)
state = {}
last_change = {}
HIGHLIGHT_SEC = 0.5
NO_HIGHLIGHT = {"soc_hi"}
...
def render(fps, last_age):
    s = state
    L = [title_bar("BESTGO Battery   500 kbps"
                   + ("   [DUMMY]" if DUMMY else ""))]
...
if DUMMY:
    ...
    if not devs:
        print("No gs_usb device found. "
              "(Use -bestgo_dummy to run without hardware.)")
        sys.exit(1)
    dev = devs[0]
    # 500 kbps: brp=20 (vs brp=40 for the EZkontrol's 250 kbps bus)
    dev.set_timing(prop_seg=1, phase_seg1=13, phase_seg2=2, sjw=2, brp=20)
    dev.start()
