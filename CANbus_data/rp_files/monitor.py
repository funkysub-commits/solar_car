...
Raspberry Pi / SocketCAN version. See pc_files/monitor.py for the
...
(0x351..0x379). One SocketCAN interface on that bus sees, and decodes, both.

Usage:
    python3 monitor.py [DURATION_SEC]
    python3 monitor.py -ezkontrol_dummy -bestgo_dummy   (no hardware at all)
    python3 monitor.py -bestgo_dummy                    (EZkontrol live only)
    python3 monitor.py -ezkontrol_dummy                 (BESTGO live only)
...
interface is opened at all. This tool does not write ASC logs; use
bestgo_decode.py / ezkontrol_decode.py to log the bus to a file.

Reads the bus through SocketCAN. Bring the interface up first (this sets
the bitrate) -- see can_up.sh. Override its name with the CAN_CHANNEL
...
def channel_loc(ch):
    ...
    return "[DUMMY]" if ch.dummy else CAN_CHANNEL


# --- panel renderers --------------------------------------------------------
def render_ez(ch):
    s, p = ch.panel.state, ch.panel
    L = [title_bar(f"EZkontrol B48800   {channel_loc(ch)}")]
...
def render_bg(ch):
    s, p = ch.panel.state, ch.panel
    L = [title_bar(f"BESTGO Battery   {channel_loc(ch)}")]
...
class Channel:
    ...
    def __init__(self, name, dummy, gen, handler, dummy_period):
        self.name = name
        self.dummy = dummy
        self.gen = gen
...
ez = Channel("EZkontrol", EZ_DUMMY, ez_dummy_frames, ez_handle, 0.1)
bg = Channel("BESTGO", BG_DUMMY, bg_dummy_frames, bg_handle, 1.0)
channels = [ez, bg]
...
def ensure_can(channel):
    ...
    if not os.path.isdir(sysdir):
        sys.exit(f"CAN interface '{channel}' not found. Plug in the SH-C31G "
                 f"and bring the bus up (see can_up.sh), or simulate both "
                 f"devices with -ezkontrol_dummy -bestgo_dummy.")
    try:
...
# --- open the shared-bus interface (unless both devices are simulated) ------
bus = None
if not (EZ_DUMMY and BG_DUMMY):
    ...
    print(f"listening on {CAN_CHANNEL} (shared 500 kbps bus), decoding: {decoded}")
